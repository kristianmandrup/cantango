...
Note that CanTango uses @autoload_modules@ from the "sweetloader":https://github.com/kristianmandrup/sweet_loader.git' gem to ensure that all files are lazy-loaded. Thus if you configure CanTango to exclude an engine, the code for that engine will never be loaded.

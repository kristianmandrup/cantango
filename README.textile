...
For the next release, we plan to optimize permits loading (via pre-registration) and increase visibility of the Ability flow, to allow one to see which rules are triggered by which permits (special :debug config mode).
